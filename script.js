...
function unlockBadges() {
  badgeList.innerHTML = "";
  const unlocked = badgePool.sort(() => 0.5 - Math.random()).slice(0, 3);
  unlocked.forEach(badge => {
    const div = document.createElement("div");
    div.style = "background:#222; padding:1rem; border-radius:8px; min-width:120px; text-align:center;";
    div.innerHTML = `<div style="font-size:2rem;">${badge.emoji}</div><p>${badge.name}</p>`;
